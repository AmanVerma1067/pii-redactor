"""Embedded Image OCR & Media Redaction Engine (Tesseract + Pillow + OpenCV).

Intercepts embedded media parts in the docx package (/word/media/), runs OCR to extract text
and bounding boxes, detects Indian identity documents (PAN, Aadhaar) and visual PII, and applies
redaction:
  - Solid black rectangles ('mask')
  - Background-matched synthetic text ('replace')
  - Gaussian blur ('blur')
  - Designated placeholder banner: '[CONFIDENTIAL ID CARD REDACTED]' ('banner')
  - Face blurring and QR code pixelation
  - Re-encodes modified image buffer back into docx XML relationship part without breaking document integrity.
"""
...
ID_KEYWORDS = (
    "income tax", "permanent account", "govt", "government of india", "aadhaar", "aadhar",
...
LABEL_RE = re.compile(
    r"(?i)\b(father'?s?\s*name|mother'?s?\s*name|husband'?s?\s*name|name|s\s*/\s*o|d\s*/\s*o|w\s*/\s*o|c\s*/\s*o)\b\s*[:\-.]?\s*"
)
...
class ImageRedactor:
    """Extracts media, performs OCR, detects identity cards, and redacts visual PII."""
    ...
    def __init__(
        ...
        self.pseudo = pseudo
        self.policy = policy  # replace | mask | blur | banner
        self.lang = lang
        self.min_side = min_side
        self.mask_qr = mask_qr
    ...
    def _bg_fg(img: Image.Image, box) -> tuple[tuple, tuple]:
        ...
        bg = tuple(med(border, c) for c in range(3)) if border else (255, 255, 255)
        dark = sorted(px, key=lambda p: sum(p))[: max(1, len(px) // 10)]
        fg = tuple(med(dark, c) for c in range(3)) if dark else (0, 0, 0)
        return bg, fg

    def _create_banner(self, img: Image.Image) -> Image.Image:
        """Create placeholder banner '[CONFIDENTIAL ID CARD REDACTED]' matching original dimensions."""
        w, h = img.size
        banner = Image.new("RGB", (w, h), color=(245, 245, 245))
        draw = ImageDraw.Draw(banner)
        draw.rectangle((10, 10, w - 10, h - 10), outline=(200, 30, 30), width=6)
        text = "[CONFIDENTIAL ID CARD REDACTED]"
        font_size = max(16, min(w // 20, h // 10))
        font = _font(font_size)
        bbox = draw.textbbox((0, 0), text, font=font)
        tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
        draw.text(((w - tw) / 2, (h - th) / 2 - 10), text, fill=(200, 30, 30), font=font)
        sub = "Statutory Indian Identity Document Masked"
        sub_font = _font(max(10, font_size // 2))
        sbbox = draw.textbbox((0, 0), sub, font=sub_font)
        stw, sth = sbbox[2] - sbbox[0], sbbox[3] - sbbox[1]
        draw.text(((w - stw) / 2, (h + th) / 2 + 10), sub, fill=(80, 80, 80), font=sub_font)
        return banner
    ...
    def _paint(self, img: Image.Image, box, fake: str) -> None:
        ...
        if self.policy == "mask":
            # Mask sensitive bounding boxes with solid black rectangles
            ...
            try:
                ...
                if ok and pts is not None:
                    ...
                    box = (
                        int(max(0, xs.min() - 8)),
                        int(max(0, ys.min() - 8)),
                        int(min(img.width, xs.max() + 8)),
                        int(min(img.height, ys.max() + 8)),
                    )
                    if self.policy == "mask":
                        draw = ImageDraw.Draw(img)
    ...
    def process_part(self, part) -> ImageReport:
        ...
        # register first (global consistency)
        for h in hits:
            self.pseudo.register(h.text, h.type)

        if self.policy == "banner" and (rep.id_document or hits):
            work = self._create_banner(work)
            for h in hits:
                rep.hits.append({
                    "type": h.type.value,
                    "text": h.text,
        ...
        if not rep.hits and not rep.qr_codes and not rep.faces and not rep.failsafe_blur and not rep.id_document:
