...
> **Scope, stated honestly.** The scores below come from the annotated **RHP-style benchmark** in this
> repo (`eval/data/benchmark_rhp.docx` + `ground_truth.json`). It reproduces the structures and entities
> listed for the KSH International RHP: promoter names, CIN, DINs, SEBI numbers, office addresses,
> contacts, and scanned PAN/Aadhaar cards. It adds deliberate hard cases and 43 false-positive traps.
> Values not given in the brief are synthetic. The benchmark was written by the same author as the
> system, so read these numbers as an **optimistic upper bound**. `eval/annotate.py` produces a real
> ground truth for the full 400+ page RHP (workflow in section 6), and those numbers are the ones to
> quote for the submission.
...
What would leak on the *real* RHP, ranked by risk:
1. People named only in running prose with no honorific or label and uncommon names (KMP bios, litigation sections). Mitigation: spaCy + review of `report.json` detections.
2. Organisations without a suffix (group companies, customers, lenders referred to by brand). Same mitigation as above.
3. Low-resolution or rotated scans. Mitigation: 2x upscale, psm-11 retry, and the fail-safe blur on ID-looking images with no hits.
4. Hindi-script names on the Aadhaar/PAN card. Mitigation: `--ocr-lang eng+hin` (installed via `packages.txt`/Dockerfile).
...
## 6. Protocol for the real RHP (to run before submission)
```bash
